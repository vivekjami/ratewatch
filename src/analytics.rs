...
impl AnalyticsManager {
    ...
    pub async fn get_stats(&self) -> anyhow::Result<Value> {
        ...
        let requests_change = if yesterday_requests > 0 {
            ((today_requests as f64 - yesterday_requests as f64) / yesterday_requests as f64)
                * 100.0
        } else {
            0.0
        };
    ...
    pub async fn get_top_keys(&self, limit: u32) -> anyhow::Result<Value> {
        ...
        for key in keys.iter().take(limit as usize * 2) {
            // Get more than needed in case some are empty
            if let Ok(stats) = conn.hgetall::<_, HashMap<String, String>>(key).await {
                ...
                let total_requests: u64 = stats
                    ...
                    .unwrap_or(0);
                let allowed_requests: u64 = stats
                    .get("allowed_requests")
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(0);
                let last_seen: u64 = stats
                    .get("last_seen")
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(0);
    ...
    pub async fn get_request_rate_data(&self, window: &str) -> anyhow::Result<Value> {
        ...
        for i in 0..data_points {
            ...
            let dt = chrono::DateTime::from_timestamp(time_point as i64, 0).unwrap_or_default();
